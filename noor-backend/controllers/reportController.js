...
exports.getOverallReport = async (req, res) => {
  try {
    ...
    const utilization =
      totalAllocated > 0 ? (totalExpenses / totalAllocated) * 100 : 0;
    ...
    const [projectFinancials] = await db.query(`
             ...
             WHERE 1=1 ${projectFilter}
        `);
    const overBudgetProjects = projectFinancials.filter(
      (p) => (p.spent || 0) > (p.allocated || 0),
    ).length;
    ...
    milestones.forEach((m) => {
      if (m.status === "Completed" || m.progress === 100) {
        ...
        m.dynamic_progress !== null &&
        m.dynamic_progress !== undefined
      ) {
    ...
    const milestoneStats = {
      ...
      latest_achievement_date: milestones
        ...
        .sort(
          (a, b) =>
            new Date(b.actual_completion_date || b.updated_at).getTime() -
            new Date(a.actual_completion_date || a.updated_at).getTime(),
        )[0]?.actual_completion_date,
    };
    ...
    const reportData = {
      ...
      taskStatistics: {
        ...taskStats[0],
        avg_completion_time_days: avgTime[0].avg_days
          ? Number(avgTime[0].avg_days).toFixed(1)
          : 0,
      },
      materialOverview: {
        ...materialSummary[0],
      ...
      employeePerformance: employeePerformance.map((e) => ({
        ...e,
        avg_completion_days: e.avg_completion_days
          ? Number(e.avg_completion_days).toFixed(1)
          : "0.0",
        performance_score:
          e.assigned_tasks > 0
            ? Math.round((e.completed_tasks / e.assigned_tasks) * 100)
            : 0,
      ...
      milestones: {
        ...
        achievements: milestones
          ...
          .map(
            (m) =>
              `Milestone '${m.name}' in project '${m.site_name}' successfully completed`,
          ),
      },
      topExpenseProjects: projectFinancials
        ...
        .map((p) => ({
          ...
          utilization:
            p.allocated > 0
              ? Math.round(((p.spent || 0) / p.allocated) * 100)
              : 0,
        })),
      actionItems: [
        ...stuckApprovals.map(
          (t) =>
            `Task '${t.name}' in '${t.project_name}' waiting for approval > 3 days`,
        ),
        taskStats[0].waiting_approval > 0
          ? `${taskStats[0].waiting_approval} tasks waiting for approval`
          : null,
        milestoneStats.delayed > 0
          ? `${milestoneStats.delayed} milestones are currently delayed`
          : null,
        overBudgetProjects > 0
          ? `${overBudgetProjects} projects have exceeded their budget`
          : null,
        zeroProgressPhases.length > 0
          ? `${zeroProgressPhases.length} phases showing zero progress`
          : null,
