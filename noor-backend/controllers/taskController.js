...
exports.addTaskUpdate = async (req, res) => {
  try {
    ...
    const [task] = await db.query(
      "SELECT progress, phase_id, site_id, name FROM tasks WHERE id = ?",
      [taskId],
    );
    ...
    await db.query("UPDATE tasks SET progress = ? WHERE id = ?", [
      progress,
      taskId,
    ]);
    ...
    if (Number(progress) === 100) {
      ...
      await db.query('UPDATE tasks SET status = "in_progress" WHERE id = ?', [
        taskId,
      ]);
    }
...
exports.sendTaskMessage = async (req, res) => {
  try {
    ...
    console.log(
      `[TaskController] sendTaskMessage: taskId=${taskId}, senderId=${senderId}, content=${content}`,
    );

    // Get site_id for the task to ensure file sync
    const [taskData] = await db.query(
      "SELECT site_id FROM tasks WHERE id = ?",
      [taskId],
    );
...
exports.completeTask = async (req, res) => {
  try {
    ...
    console.log(
      `[TaskController] Completing task ${taskId} by user ${employeeId}. Sending notification...`,
    );
    await db.query(
    ...
    if (taskData.length > 0) {
      ...
      await db.query(
        ...
        [
          ...
          task.phase_id,
          taskId,
          `Task "${task.name}" submitted for approval`,
        ],
      );
    ...
    require("fs").appendFileSync(
      "error_log.txt",
      `[${new Date().toISOString()}] Error in completeTask: ${error.message}\n${error.stack}\n`,
    );
    res
      .status(500)
...
exports.approveTask = async (req, res) => {
  try {
    ...
    const [taskData] = await db.query(
      "SELECT site_id, phase_id, name, completed_at FROM tasks WHERE id = ?",
      [taskId],
    );
    if (taskData.length > 0) {
      const task = taskData[0];
      ...
      for (const assignee of assignees) {
        await db.query(
          ...
          [
            ...
            `Your work on "${task.name}" has been approved!`,
          ],
        );
      ...
      for (const assignee of assignees) {
        await db.query(
          ...
          [
            ...
            taskId,
            assignee.employee_id,
            `Changes requested for "${task.name}". Check chat for details.`,
          ],
        );
      }
  ...
  try {
    ...
    if (existing.length > 0) {
      ...
      if (dueDate) {
        ...
        await db.query("UPDATE tasks SET due_date = ? WHERE id = ?", [
          formattedDate,
          taskId,
        ]);
      }
...
exports.updateTask = async (req, res) => {
  try {
    ...
    const { name, status, start_date, due_date, amount, assigned_to } =
      req.body;
    ...
    await db.query(
      ...
      [
        name,
        status,
        ensureIsoDate(start_date),
        ensureIsoDate(due_date),
        amount,
        taskId,
      ],
    );
    ...
    if (assigned_to !== undefined) {
      ...
      await db.query("DELETE FROM task_assignments WHERE task_id = ?", [
        taskId,
      ]);
