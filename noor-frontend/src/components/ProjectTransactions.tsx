...
const ProjectTransactions: React.FC<ProjectTransactionsProps> = ({
  siteId,
  phases,
  readonly = false,
  clientName,
  ...
  const [viewPhaseTransactions, setViewPhaseTransactions] = useState<
    Transaction[]
  >([]);
  ...
  const submitTransactionWithArgs = async (
    data: any,
    force: boolean = false,
  ) => {
    try {
      ...
      if (!force && data.type === "OUT" && data.phase_id) {
        console.log(
          "[ProjectTransactions] Checking budget for phase:",
          data.phase_id,
        );
      ...
      console.log(
        "[ProjectTransactions] Proceeding with transaction submission (force=" +
          force +
          ")",
      );
          ...
          <View style={styles.section}>
            ...
            {pTrans.length === 0 ? (
              <Text style={styles.emptyText}>
                No transactions for this phase.
              </Text>
            ) : (
              pTrans.map((item) => (
                <View key={item.id} style={styles.transactionRow}>
                  ...
                  <View style={styles.transInfo}>
                    ...
                    <View style={styles.transMeta}>
                      ...
                      {item.type === "IN" && (
                        <Text style={[styles.transPhase, { color: "#166534" }]}>
                          {" "}
                          • Payment In
                        </Text>
                      )}
                      {item.type === "OUT" && (
                        <Text style={[styles.transPhase, { color: "#DC2626" }]}>
                          {" "}
                          • Expense
                        </Text>
                      )}
  ...
  return (
    <View style={styles.container}>
                      ...
                      <View
                        style={[
                          styles.progressBarFill,
                          {
                            ...
                            backgroundColor: isOver ? "#DC2626" : "#166534",
                          },
                        ]}
                      />
                    </View>
      ...
      {/* Action Buttons - Viewport Fixed at Bottom */}
      {!readonly && (
        <View style={styles.fixedActionButtonsContainer}>
          <TouchableOpacity
            style={[styles.fixedActionButton, { backgroundColor: "#059669" }]} // Teal/Emerald
            onPress={() => {
              setType("IN");
          ...
          <TouchableOpacity
            style={[styles.fixedActionButton, { backgroundColor: "#BE123C" }]} // Rose/Pink
            onPress={() => {
              setType("OUT");
              setAddModalVisible(true);
            }}
                ...
                <View style={styles.methodSelector}>
                  {["Cash", "Bank Transfer", "Cheque"].map((method) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.methodChip,
                        paymentMethod === method && styles.methodChipSelected,
                      ]}
            ...
            {warningModalVisible && (
              <View style={styles.warningOverlay}>
                <View style={styles.warningContent}>
                  ...
                  <Text style={styles.warningNewTotal}>
                    {warningMessage.newTotal}
                  </Text>
...
const styles = StyleSheet.create({
  ...
  scrollContentContainer: {
    paddingBottom: 20, // Space so content doesn't hide behind fixed buttons
  },
  section: {
    marginBottom: 24,
  },
  ...
  // Fixed Action Buttons at Bottom (Viewport Fixed)
  fixedActionButtonsContainer: {
    ...
    paddingBottom:
      Platform.OS === "web"
        ? 16
        : Math.max(24, (Platform.OS === "android" ? 16 : 0) + 8),
    backgroundColor: "#FFF",
    borderTopWidth: 1,
  ...
  fixedActionButton: {
    ...
    justifyContent: "center",
    elevation: 3,
    ...(Platform.OS !== "web" && { width: "100%" }), // Full width on mobile
  },
