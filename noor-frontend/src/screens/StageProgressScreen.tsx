    ...
    <View style={styles.container}>
      ...
      <View style={{ flex: 1, marginLeft: 8 }}>
        <View
          style={{
            height: 3,
            backgroundColor: styles.progressBarConfig.bg,
            borderRadius: 2,
            overflow: "hidden",
            marginBottom: 4,
          }}
...
const StageProgressScreen = ({ route, navigation }: any) => {
  ...
  useEffect(() => {
    if (phaseId || taskId) {
      console.log("[StageProgress] useEffect triggered for:", {
        phaseId,
        taskId,
      });
      fetchData();
  ...
  const fetchData = async () => {
    try {
      ...
      if (taskId) {
        ...
        console.log(
          "[StageProgress] Response received for task:",
        ...
        const combinedMessages = [...taskMessages, ...stageMessages].sort(
          (a, b) =>
        ...
        // STAGE MODE (Legacy)
        console.log(
          `[StageProgress] Fetching Phase Details for ID: ${phaseId}`,
        );
        ...
        if (!phaseData.due_date && tasks.length > 0) {
          const sortedDues = tasks
            .map((t: any) => t.due_date)
            .filter(Boolean)
            .sort();
          if (sortedDues.length > 0)
            phaseData.due_date = sortedDues[sortedDues.length - 1];
        }
        ...
        const myTasks = tasks.filter((task: any) => {
          ...
          return (
            task.assignments &&
            task.assignments.some((a: any) => String(a.id) === String(user?.id))
          );
        });
        ...
        const uniqueUpdates = allUpdates.filter(
          (update: any, index: number, arr: any[]) => {
            if (index === 0) return true;
            const prev = arr[index - 1];
            // Remove if same message and same progress values as previous
            return !(
  ...
  const isAssigned =
    ...
    (user?.id &&
      phase?.assigned_to &&
      String(phase.assigned_to) === String(user.id)) ||
    (subTasks &&
      subTasks.some(
        (t) =>
          t.assignments &&
          t.assignments.some((a: any) => String(a.id) === String(user?.id)),
      ));
  ...
  const handleCompleteTask = async (task: any) => {
    ...
    if (!isAdmin && !userIsAssigned) {
      Alert.alert(
        "Restricted Access",
        `This task is restricted to assigned employees only.`,
      );
      return;
    }

    if (task.status === "Completed" || task.status === "waiting_for_approval")
      return; // Already done or pending
      ...
      async () => {
        try {
          ...
          setSubTasks((prev) =>
            prev.map((t) =>
              t.id === task.id
                ? { ...t, status: "waiting_for_approval", progress: 100 }
                : t,
            ),
          );
  ...
  const handleRejectTask = async (task: any) => {
    confirmAction(
      ...
      async () => {
        try {
          await api.put(`/tasks/${task.id}/reject`, {
            reason: "Admin requested changes",
          });
          setSubTasks((prev: any[]) =>
            prev.map((t) =>
          ...
          Alert.alert(
            "Success",
            "Changes requested. Task reverted to In Progress.",
          );
        } catch (error) {
  ...
  const submitReject = async () => {
    ...
    try {
      if (taskId) {
        ...
        await api.put(`/phases/${currentPhaseId}/reject`, {
          reason: changeRequestReason,
        });
        Alert.alert(
          "Success",
          "Changes requested. Phase sent back for revisions",
        );
      }
  ...
  const handleAddTodo = async () => {
    if (!isAssigned) {
      Alert.alert(
        "Action Restricted",
        "You must assign an employee to add todos.",
      );
      return;
    }
  ...
  const handleToggleTodo = async (todoId: number) => {
    if (!isAssigned) {
      Alert.alert(
        "Action Restricted",
        "You must assign an employee to update todos.",
      );
      return;
    }
    try {
  ...
  const handleSendMessage = async (customContent?: string) => {
    console.log("[StageProgress] handleSendMessage called", {
      customContent,
      messageText,
    });
    const contentToSend =
      customContent !== undefined ? customContent : messageText;
  ...
  const renderFeed = () => {
    const feed = [
      ...
      .filter((item) => {
        ...
        if (
          item.message &&
          item.message.includes("Work completed - Ready for admin approval")
        )
          return false;
        if (
          item.content &&
          item.content.includes("Work completed - Ready for admin approval")
        )
          return false;
        // Also filter out generic auto-generated updates if wanted
        if (item.itemType === "update" && item.message === "Progress Update")
          return false;
        return true;
      })
      .sort(
      ...
      >
        ...
        <View style={styles.sectionCard}>
          ...
          {subTasks.map((task, index) => {
            const userIsAssigned =
              task.assignments &&
              task.assignments.some((a: any) => a.id === user?.id);
              ...
              >
                <View style={{ flex: 1 }}>
                  <Text
                    style={[
                      styles.todoText,
                      task.status === "Completed" && styles.todoTextDone,
                    ]}
                  >
                    {task.name}
                  </Text>
          ...
          {subTasks.length === 0 && (
            <Text style={styles.emptyTextSimple}>No tasks defined</Text>
          )}
        </View>
      ...
      {user ? (
          ...
          >
            <View style={styles.inputArea}>
              ...
              <TouchableOpacity
                ...
                onLongPress={
                  !messageText.trim() && !mediaUri ? startRecording : undefined
                }
        ...
        >
          ...
          <Text style={{ color: "#DC2626", fontWeight: "600", fontSize: 14 }}>
            Assign an employee to start progress
          </Text>
        </View>
      )}
                  ...
                  <View style={{ gap: 8, alignItems: "center" }}>
                    ...
                    >
                      <Ionicons
                        ...
                        size={16}
                        color={isPlayingPreview ? "#047857" : "#374151"}
                      />
                      <Text
                        style={{
                          fontSize: 12,
