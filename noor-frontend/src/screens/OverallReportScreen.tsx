...
import { View, Text, ScrollView, TouchableOpacity, StyleSheet, ActivityIndicator, Alert, Platform, Modal, useWindowDimensions } from 'react-native';
...
const DetailItem = ({ label, value, color, suffix }: { label: string, value: string | number, color?: string, suffix?: string }) => (
    <View style={styles.detailItem}>
        <Text style={styles.detailValue}>{value}{suffix}</Text>
        <Text style={[styles.detailLabel, color ? { color } : {}]}>{label}</Text>
    </View>
);

const ExecutiveCard = ({ label, value, icon, color, subLabel }: { label: string, value: number | string, icon: any, color: string, subLabel?: string }) => (
...
const ProjectHealthRow = ({ project }: { project: any }) => {
    ...
    if (project.status === 'delayed' || project.days_behind !== '0') {
        ...
        statusLabel = 'Attention'; // Changed label to fit logic nicely
        statusBg = '#FFFBEB';
    }
        ...
        <View style={styles.projectRow}>
            ...
            <View style={styles.projectLeft}>
                <View style={styles.projectHeader}>
                    ...
                    <Text style={styles.projectLoc}><Ionicons name="location-outline" size={12} /> {project.location}</Text>
                </View>
            ...
            <View style={styles.projectRight}>
                ...
                {project.pending_approvals > 0 ? (
                    <View style={styles.pendingActionBadge}>
                        <Text style={styles.pendingActionText}>{project.pending_approvals} Actions Pending</Text>
                    </View>
                ) : (
    ...
    return (
        <View style={styles.employeeRow}>
            ...
            <View style={styles.empStatsCol}>
                ...
                <View style={styles.empStatItem}>
                    <Text style={[styles.empStatVal, { color: '#10B981' }]}>{employee.completed_tasks}</Text>
                    <Text style={styles.empStatLabel}>Done</Text>
                </View>
                <View style={styles.empStatItem}>
                    <Text style={[styles.empStatVal, { color: '#F59E0B' }]}>{employee.pending_tasks}</Text>
                    <Text style={styles.empStatLabel}>Pending</Text>
                </View>
                <View style={styles.empStatItem}>
                    <Text style={[styles.empStatVal, { color: '#EF4444' }]}>{employee.overdue_tasks}</Text>
            ...
            <View style={styles.empRatingCol}>
                <View style={[styles.ratingPill, { backgroundColor: rating.color + '15' }]}>
...
const SectionHeader = ({ title, icon }: { title: string, icon?: any }) => (
    <View style={styles.sectionHeader}>
        {icon && <Ionicons name={icon} size={20} color="#374151" style={{ marginRight: 8 }} />}
...
const OverallReportScreen = ({ navigation }: any) => {
    ...
    const generateHTML = () => {
        ...
        const dateRange = fromDate && toDate ? `${fromDate.toLocaleDateString()} - ${toDate.toLocaleDateString()}` : 'All Time';
        ...
        return `
            ...
            <body>
                ...
                <div class="alert-box">
                    ...
                    ${report.actionItems.map((item: string) => `
                ...
                <div class="section">
                    ...
                    <table>
                        ...
                        <tbody>
                            ${report.projectSummary.slice(0, 8).map((p: any) => `
                                <tr>
                                    <td><b>${p.name}</b></td>
                                    <td>${p.location}</td>
                                    <td>
                ...
                <div class="section">
                    ...
                    <table style="width: 100%; margin-bottom: 20px;">
                     ...
                     <table>
                        ...
                        ${(report.topExpenseProjects || []).slice(0, 5).map((e: any) => `
                            <tr>
                                <td>${e.name}</td>
                ...
                <div class="section">
                    ...
                    <table>
                        ...
                        <tbody>
                            ${report.employeePerformance.slice(0, 8).map((e: any) => `
                                <tr>
                                    <td><b>${e.name}</b></td>
    ...
    const handleDownloadPDF = async () => {
        ...
        } catch (error) { Alert.alert('Error', 'Failed PDF'); }
    };
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.sectionContainer}>
                    ...
                    <View style={styles.executiveGrid}>
                        <ExecutiveCard label="Active Projects" value={report.companyOverview.active_projects} icon="business" color="#3B82F6" />
                        <ExecutiveCard label="Completed" value={report.companyOverview.completed_projects} icon="checkmark-done" color="#10B981" />
                        <ExecutiveCard label="Delayed" value={report.companyOverview.projects_with_delays} icon="alert-circle" color="#EF4444" />
                        <ExecutiveCard label="Budget Util." value={`${report.financialSummary.utilization_percentage}%`} icon="wallet" color="#F59E0B" />
                    </View>
                </View>
                ...
                <View style={styles.sectionContainer}>
                    ...
                    <View style={styles.projectList}>
                        {report.projectSummary.map(p => <ProjectHealthRow key={p.id} project={p} />)}
                    </View>
                </View>
                ...
                <View style={styles.sectionContainer}>
                    ...
                    <View style={styles.milestoneGrid}>
                        <View style={styles.milestoneStat}>
                            <Text style={styles.mileVal}>{report.milestones.stats.completed}</Text>
                            <Text style={[styles.mileLabel, { color: '#10B981' }]}>Completed</Text>
                        </View>
                        <View style={styles.milestoneStat}>
                            <Text style={styles.mileVal}>{report.milestones.stats.in_progress}</Text>
                            <Text style={[styles.mileLabel, { color: '#3B82F6' }]}>Ongoing</Text>
                        </View>
                        <View style={styles.milestoneStat}>
                            <Text style={styles.mileVal}>{report.milestones.stats.delayed}</Text>
                            <Text style={[styles.mileLabel, { color: '#EF4444' }]}>Delayed</Text>
                        </View>
                    </View>
                </View>
                ...
                <View style={styles.sectionContainer}>
                    ...
                    <View style={styles.taskStatsGrid}>
                        <View style={styles.taskStatBox}>
                            <Text style={styles.taskVal}>{report.taskStatistics.total_tasks}</Text>
                            <Text style={styles.taskLabel}>Total Tasks</Text>
                        </View>
                        <View style={styles.taskStatBox}>
                            <Text style={[styles.taskVal, { color: '#10B981' }]}>{report.taskStatistics.completed_tasks}</Text>
                            <Text style={styles.taskLabel}>Completed</Text>
                        </View>
                        <View style={styles.taskStatBox}>
                            <Text style={[styles.taskVal, { color: '#F59E0B' }]}>{report.taskStatistics.waiting_approval}</Text>
                            <Text style={styles.taskLabel}>Pending Approval</Text>
                        </View>
                        <View style={styles.taskStatBox}>
                            <Text style={styles.taskVal}>{report.taskStatistics.avg_completion_time_days}d</Text>
                ...
                <View style={styles.sectionContainer}>
                    ...
                    <View style={styles.employeeList}>
                        {getSortedEmployees().length > 0 ? (
                            getSortedEmployees().map(e => <EmployeeRow key={e.id} employee={e} />)
                        ) : (
...
const styles = StyleSheet.create({
    ...
    detailItem: {},
    detailValue: { fontSize: 16, fontWeight: 'bold', color: '#1F2937', marginBottom: 2 },
    ...
    taskVal: { fontSize: 18, fontWeight: 'bold', color: '#1F2937', marginBottom: 4 },
    ...
    empStatsCol: { flex: 1, flexDirection: 'row', justifyContent: 'space-around' },
